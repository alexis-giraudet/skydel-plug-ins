...
public:
  ...
  ChangeModulationTargetName() : CommandBase(CmdName, TargetId) {}

  ChangeModulationTargetName(const std::string& targetName, const std::string& id) : CommandBase(CmdName, TargetId)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["TargetName"]) &&
  ...
  std::string targetName() const { return parse_json<std::string>::parse(m_values["TargetName"]); }

  void setTargetName(const std::string& targetName)
  {
    m_values.AddMember("TargetName",
                       parse_json<std::string>::format(targetName, m_values.GetAllocator()),
