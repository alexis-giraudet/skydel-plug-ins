...
public:
  ...
  CopyVehicleAntennaModel() : CommandBase(CmdName, TargetId) {}

  CopyVehicleAntennaModel(const std::string& name, const std::string& copyName) : CommandBase(CmdName, TargetId)
  {
  ...
  std::string copyName() const { return parse_json<std::string>::parse(m_values["CopyName"]); }

  void setCopyName(const std::string& copyName)
  {
    m_values.AddMember("CopyName",
                       parse_json<std::string>::format(copyName, m_values.GetAllocator()),
