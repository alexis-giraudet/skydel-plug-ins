...
public:
  ...
  DataFolderResult() : CommandResult(CmdName, TargetId) {}

  DataFolderResult(const std::string& folder) : CommandResult(CmdName, TargetId) { setFolder(folder); }
  ...
  static DataFolderResultPtr create(const std::string& folder) { return std::make_shared<DataFolderResult>(folder); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["Folder"]);
  }
  ...
  std::string folder() const { return parse_json<std::string>::parse(m_values["Folder"]); }

  void setFolder(const std::string& folder)
  {
    m_values.AddMember("Folder",
                       parse_json<std::string>::format(folder, m_values.GetAllocator()),
