...
{
///
/// Adjusts the Advanced Jammer IQ data amplitude on the Dektec output by adding noise to ensure power level linearity
/// on weak jammer signals. Note that enabling this option will increase the noise observed at the RF output for all
/// jammer signal power levels.
///
...
public:
  ...
  EnableDektecAdjustedNoise() : CommandBase(CmdName, TargetId) {}

  EnableDektecAdjustedNoise(bool enable) : CommandBase(CmdName, TargetId) { setEnable(enable); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<bool>::is_valid(m_values["Enable"]);
  }
  ...
  bool enable() const { return parse_json<bool>::parse(m_values["Enable"]); }
