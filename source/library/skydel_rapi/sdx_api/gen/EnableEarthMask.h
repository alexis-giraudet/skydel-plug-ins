...
public:
  ...
  EnableEarthMask() : CommandBase(CmdName, TargetId) {}

  EnableEarthMask(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static EnableEarthMaskPtr create(bool enabled) { return std::make_shared<EnableEarthMask>(enabled); }

  static EnableEarthMaskPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<EnableEarthMask>(ptr); }
