...
public:
  ...
  EnableLogRaw() : CommandBase(CmdName, TargetId) {}

  EnableLogRaw(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static EnableLogRawPtr create(bool enabled) { return std::make_shared<EnableLogRaw>(enabled); }

  static EnableLogRawPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<EnableLogRaw>(ptr); }
