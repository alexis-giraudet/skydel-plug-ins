...
public:
  ...
  EnableLogRinex() : CommandBase(CmdName, TargetId) {}

  EnableLogRinex(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static EnableLogRinexPtr create(bool enabled) { return std::make_shared<EnableLogRinex>(enabled); }

  static EnableLogRinexPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<EnableLogRinex>(ptr); }
