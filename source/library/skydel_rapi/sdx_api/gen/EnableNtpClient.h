...
public:
  ...
  EnableNtpClient() : CommandBase(CmdName, TargetId) {}

  EnableNtpClient(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static EnableNtpClientPtr create(bool enabled) { return std::make_shared<EnableNtpClient>(enabled); }

  static EnableNtpClientPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<EnableNtpClient>(ptr); }
