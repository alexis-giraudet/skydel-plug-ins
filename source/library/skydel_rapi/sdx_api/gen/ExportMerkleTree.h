...
/// ---------------- ---------
/// ---------------------------------------------------------------------------------------------------------- Id string
/// Merkle Tree unique identifier. PublicKeyIndexes array int Indexes (0-based) of Public Keys to export in the Merkle
/// Tree. If empty, all Public Keys will be exported. FilePath         string    Export file path for the Merkle Tree.
...
public:
  ...
  ExportMerkleTree() : CommandBase(CmdName, TargetId) {}

  ExportMerkleTree(const std::string& id,
  ...
  static ExportMerkleTreePtr create(const std::string& id,
                                    const std::vector<int>& publicKeyIndexes,
                                    const std::string& filePath,
                                    bool overwriteFile)
  {
  ...
  bool overwriteFile() const { return parse_json<bool>::parse(m_values["OverwriteFile"]); }

  void setOverwriteFile(bool overwriteFile)
  {
    m_values.AddMember("OverwriteFile",
                       parse_json<bool>::format(overwriteFile, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
