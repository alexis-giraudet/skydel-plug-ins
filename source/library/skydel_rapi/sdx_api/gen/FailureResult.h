...
public:
  ...
  FailureResult() : CommandResult(CmdName, TargetId) {}

  FailureResult(const std::string& errorMsg) : CommandResult(CmdName, TargetId) { setErrorMsg(errorMsg); }
  ...
  static FailureResultPtr create(const std::string& errorMsg) { return std::make_shared<FailureResult>(errorMsg); }
  ...
  static FailureResultPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<FailureResult>(ptr); }

  virtual bool isValid() const override
  {

    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["ErrorMsg"]);
  }
  ...
  std::string errorMsg() const { return parse_json<std::string>::parse(m_values["ErrorMsg"]); }

  void setErrorMsg(const std::string& errorMsg)
  {
    m_values.AddMember("ErrorMsg",
                       parse_json<std::string>::format(errorMsg, m_values.GetAllocator()),
