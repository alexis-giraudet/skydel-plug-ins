...
public:
  ...
  ForceAttitudeToZero() : CommandBase(CmdName, TargetId) {}

  ForceAttitudeToZero(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static ForceAttitudeToZeroPtr create(bool enabled) { return std::make_shared<ForceAttitudeToZero>(enabled); }
