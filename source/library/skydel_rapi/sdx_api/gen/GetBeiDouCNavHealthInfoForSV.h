...
namespace Sdx
{
  namespace Cmd
  {
    ...
    {
    public:
      inline static const char* const CmdName = "GetBeiDouCNavHealthInfoForSV";
      inline static const char* const Documentation = "Get BeiDou CNav satellite health info\n"      "\n"      "Name        Type            Description\n"      "----------- --------------- -------------------------------------------------------------------------------------------\n"      "SvId        int             Satellite SV ID 1..35, or use 0 to apply new value to all satellites.\n"      "DataSetName optional string Optional name of the data set to use. If no value is provided, the active data set is used.";
    ...
    REGISTER_COMMAND_TO_FACTORY(GetBeiDouCNavHealthInfoForSV);
  }
}
