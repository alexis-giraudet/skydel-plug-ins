...
public:
  ...
  GetConfigPathsResult() : CommandResult(CmdName, TargetId) {}

  GetConfigPathsResult(const std::vector<std::string>& paths) : CommandResult(CmdName, TargetId) { setPaths(paths); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::vector<std::string>>::is_valid(m_values["Paths"]);
  }
  ...
  std::vector<std::string> paths() const { return parse_json<std::vector<std::string>>::parse(m_values["Paths"]); }

  void setPaths(const std::vector<std::string>& paths)
  {
    m_values.AddMember("Paths",
                       parse_json<std::vector<std::string>>::format(paths, m_values.GetAllocator()),
