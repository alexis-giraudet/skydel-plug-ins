...
/// ----------- ---------------------
/// ------------------------------------------------------------------------------------------- System      string
/// "GPS", "Galileo", "BeiDou", "QZSS", "NavIC" or "PULSAR". SvId        int                   The Satellite SV ID, or
/// use 0 to apply new value to all satellites. ParamName   string                Parameter name (see table above for
/// accepted names). Val         double or int or bool Parameter value (see table above for unit and type). DataSetName
...
public:
  ...
  GetConstellationParameterForSVResult() : CommandResult(CmdName, TargetId) {}

  GetConstellationParameterForSVResult(const std::string& system,
                                       int svId,
  ...
  GetConstellationParameterForSVResult(CommandBasePtr relatedCommand,
  ...
  static GetConstellationParameterForSVResultPtr create(const std::string& system,
                                                        int svId,
                                                        const std::string& paramName,
                                                        double val,
                                                        const std::optional<std::string>& dataSetName = {})
  ...
  static GetConstellationParameterForSVResultPtr create(CommandBasePtr relatedCommand,
                                                        const std::string& system,
                                                        int svId,
                                                        const std::string& paramName,
                                                        double val,
                                                        const std::optional<std::string>& dataSetName = {})
  {
    return std::make_shared<GetConstellationParameterForSVResult>(relatedCommand,
                                                                  system,
                                                                  svId,
                                                                  paramName,
                                                                  val,
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["System"]) &&
           ...
           (parse_json<double>::is_valid(m_values["Val"]) || parse_json<int>::is_valid(m_values["Val"]) ||
