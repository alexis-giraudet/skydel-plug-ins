...
public:
  ...
  GetCurrentConfigPathResult() : CommandResult(CmdName, TargetId) {}

  GetCurrentConfigPathResult(const std::string& configPath) : CommandResult(CmdName, TargetId)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["ConfigPath"]);
  }
  ...
  std::string configPath() const { return parse_json<std::string>::parse(m_values["ConfigPath"]); }

  void setConfigPath(const std::string& configPath)
  {
    m_values.AddMember("ConfigPath",
                       parse_json<std::string>::format(configPath, m_values.GetAllocator()),
