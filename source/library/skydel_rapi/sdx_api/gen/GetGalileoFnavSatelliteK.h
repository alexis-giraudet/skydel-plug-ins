...
public:
  ...
  GetGalileoFnavSatelliteK() : CommandBase(CmdName, TargetId) {}

  GetGalileoFnavSatelliteK(int prn) : CommandBase(CmdName, TargetId) { setPrn(prn); }

  static GetGalileoFnavSatelliteKPtr create(int prn) { return std::make_shared<GetGalileoFnavSatelliteK>(prn); }
  ...
  virtual bool isValid() const override { return m_values.IsObject() && parse_json<int>::is_valid(m_values["Prn"]); }
