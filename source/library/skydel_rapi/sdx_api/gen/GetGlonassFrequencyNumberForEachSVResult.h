...
/// --------------- ---------
/// -------------------------------------------------------------------------------------------------- FrequencyNumber
/// array int Frequency number of each satellite. Zero based index (index 0 => SV ID 1, index 1 => SV ID 2, etc)
///
...
public:
  ...
  GetGlonassFrequencyNumberForEachSVResult() : CommandResult(CmdName, TargetId) {}

  GetGlonassFrequencyNumberForEachSVResult(const std::vector<int>& frequencyNumber) : CommandResult(CmdName, TargetId)
  {
  ...
  static GetGlonassFrequencyNumberForEachSVResultPtr create(CommandBasePtr relatedCommand,
                                                            const std::vector<int>& frequencyNumber)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::vector<int>>::is_valid(m_values["FrequencyNumber"]);
  }
  ...
  std::vector<int> frequencyNumber() const { return parse_json<std::vector<int>>::parse(m_values["FrequencyNumber"]); }

  void setFrequencyNumber(const std::vector<int>& frequencyNumber)
  {
    m_values.AddMember("FrequencyNumber",
                       parse_json<std::vector<int>>::format(frequencyNumber, m_values.GetAllocator()),
