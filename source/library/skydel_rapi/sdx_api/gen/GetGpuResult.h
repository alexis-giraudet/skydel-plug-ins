...
public:
  ...
  GetGpuResult() : CommandResult(CmdName, TargetId) {}

  GetGpuResult(int gpuIdx, int output, const std::string& id) : CommandResult(CmdName, TargetId)
  {
  ...
  static GetGpuResultPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetGpuResult>(ptr); }

  virtual bool isValid() const override
  {

    return m_values.IsObject() && parse_json<int>::is_valid(m_values["GpuIdx"]) &&
           parse_json<int>::is_valid(m_values["Output"]) && parse_json<std::string>::is_valid(m_values["Id"]);
  }
