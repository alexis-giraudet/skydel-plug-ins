...
public:
  ...
  GetIntTxAntenna() : CommandBase(CmdName, TargetId) {}

  GetIntTxAntenna(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetIntTxAntennaPtr create(const std::string& id) { return std::make_shared<GetIntTxAntenna>(id); }

  static GetIntTxAntennaPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetIntTxAntenna>(ptr); }
