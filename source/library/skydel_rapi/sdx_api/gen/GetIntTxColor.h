...
public:
  ...
  GetIntTxColor() : CommandBase(CmdName, TargetId) {}

  GetIntTxColor(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetIntTxColorPtr create(const std::string& id) { return std::make_shared<GetIntTxColor>(id); }

  static GetIntTxColorPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetIntTxColor>(ptr); }
