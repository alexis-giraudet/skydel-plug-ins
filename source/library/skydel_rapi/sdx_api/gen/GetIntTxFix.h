...
public:
  ...
  GetIntTxFix() : CommandBase(CmdName, TargetId) {}

  GetIntTxFix(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetIntTxFixPtr create(const std::string& id) { return std::make_shared<GetIntTxFix>(id); }

  static GetIntTxFixPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetIntTxFix>(ptr); }
