...
public:
  ...
  GetIntTxFixEcef() : CommandBase(CmdName, TargetId) {}

  GetIntTxFixEcef(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetIntTxFixEcefPtr create(const std::string& id) { return std::make_shared<GetIntTxFixEcef>(id); }

  static GetIntTxFixEcefPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetIntTxFixEcef>(ptr); }
