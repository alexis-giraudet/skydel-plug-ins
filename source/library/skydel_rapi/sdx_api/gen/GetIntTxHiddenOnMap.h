...
public:
  ...
  GetIntTxHiddenOnMap() : CommandBase(CmdName, TargetId) {}

  GetIntTxHiddenOnMap(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetIntTxHiddenOnMapPtr create(const std::string& id) { return std::make_shared<GetIntTxHiddenOnMap>(id); }
