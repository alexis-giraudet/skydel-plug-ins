...
{
///
/// Get Mask flags in the ionospheric grid for a given service provider. The array is zero based, the index 0 in a band
/// array is for the IGP with an index 1, etc.
///
...
public:
  ...
  GetIonoGridMaskAll() : CommandBase(CmdName, TargetId) {}

  GetIonoGridMaskAll(const std::string& serviceProvider) : CommandBase(CmdName, TargetId)
  {
