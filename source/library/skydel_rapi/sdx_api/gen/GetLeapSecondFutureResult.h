...
public:
  ...
  GetLeapSecondFutureResult() : CommandResult(CmdName, TargetId) {}

  GetLeapSecondFutureResult(bool enabled, int seconds, const Sdx::Date& date) : CommandResult(CmdName, TargetId)
  {
  ...
  static GetLeapSecondFutureResultPtr create(CommandBasePtr relatedCommand,
                                             bool enabled,
                                             int seconds,
                                             const Sdx::Date& date)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<bool>::is_valid(m_values["Enabled"]) &&
           parse_json<int>::is_valid(m_values["Seconds"]) && parse_json<Sdx::Date>::is_valid(m_values["Date"]);
  }
  ...
  Sdx::Date date() const { return parse_json<Sdx::Date>::parse(m_values["Date"]); }
