...
/// Key with index 0 will be exported. MerkleTreeXML    string    Merkle Tree XML representation.
///
...
public:
  ...
  GetMerkleTreeXMLResult() : CommandResult(CmdName, TargetId) {}

  GetMerkleTreeXMLResult(const std::string& id,
                         const std::vector<int>& publicKeyIndexes,
                         const std::string& merkleTreeXML) :
  ...
  GetMerkleTreeXMLResult(CommandBasePtr relatedCommand,
                         const std::string& id,
                         const std::vector<int>& publicKeyIndexes,
                         const std::string& merkleTreeXML) :
  ...
  static GetMerkleTreeXMLResultPtr create(const std::string& id,
                                          const std::vector<int>& publicKeyIndexes,
                                          const std::string& merkleTreeXML)
  {
  ...
  static GetMerkleTreeXMLResultPtr create(CommandBasePtr relatedCommand,
                                          const std::string& id,
                                          const std::vector<int>& publicKeyIndexes,
                                          const std::string& merkleTreeXML)
  {
  ...
  std::string merkleTreeXML() const { return parse_json<std::string>::parse(m_values["MerkleTreeXML"]); }

  void setMerkleTreeXML(const std::string& merkleTreeXML)
  {
    m_values.AddMember("MerkleTreeXML",
                       parse_json<std::string>::format(merkleTreeXML, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
