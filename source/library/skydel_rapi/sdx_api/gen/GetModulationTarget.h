...
public:
  ...
  GetModulationTarget() : CommandBase(CmdName, TargetId) {}

  GetModulationTarget(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetModulationTargetPtr create(const std::string& id) { return std::make_shared<GetModulationTarget>(id); }
