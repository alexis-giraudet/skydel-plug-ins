...
{
///
/// Get the IQ file to playback on the specified target and output. Skydel tries to keep the sampling rate as low as
...
public:
  ...
  GetModulationTargetIQPlayback() : CommandBase(CmdName, TargetId) {}

  GetModulationTargetIQPlayback(int output, const std::string& id) : CommandBase(CmdName, TargetId)
  {
