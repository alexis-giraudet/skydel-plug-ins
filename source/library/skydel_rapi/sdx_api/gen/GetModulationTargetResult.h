...
/// --------------- ------
/// ------------------------------------------------------------------------------------------------------------------------------------------------
/// Type            string Target type can be "DTA-2116", "DTA-2115B", "File", "N310", "None", "NoneRT", "X300" or
/// "Wavefront Controller". Path            string File path. Optional, use only if type is "File". Address string
/// Optional. IP Address if type is "N310" or "X300". ClockIsExternal bool   Indicate 10 MHz reference clock is external
/// (true) or internal (false). Optional, use only if type is "DTA-2116", "DTA-2115B", "N310" or "X300". Id string
/// Unique identifier automatically set by simulator
///
...
public:
  ...
  GetModulationTargetResult() : CommandResult(CmdName, TargetId) {}

  GetModulationTargetResult(const std::string& type,
                            const std::string& path,
                            const std::string& address,
                            bool clockIsExternal,
                            const std::string& id) :
  ...
  GetModulationTargetResult(CommandBasePtr relatedCommand,
  ...
  static GetModulationTargetResultPtr create(const std::string& type,
                                             const std::string& path,
                                             const std::string& address,
                                             bool clockIsExternal,
                                             const std::string& id)
  ...
  static GetModulationTargetResultPtr create(CommandBasePtr relatedCommand,
                                             const std::string& type,
                                             const std::string& path,
                                             const std::string& address,
                                             bool clockIsExternal,
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["Type"]) &&
           ...
           parse_json<bool>::is_valid(m_values["ClockIsExternal"]) && parse_json<std::string>::is_valid(m_values["Id"]);
  }
  ...
  bool clockIsExternal() const { return parse_json<bool>::parse(m_values["ClockIsExternal"]); }

  void setClockIsExternal(bool clockIsExternal)
  {
    m_values.AddMember("ClockIsExternal",
                       parse_json<bool>::format(clockIsExternal, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
