...
public:
  ...
  GetMultipathForID() : CommandBase(CmdName, TargetId) {}

  GetMultipathForID(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetMultipathForIDPtr create(const std::string& id) { return std::make_shared<GetMultipathForID>(id); }
