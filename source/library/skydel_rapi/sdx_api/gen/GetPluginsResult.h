...
public:
  ...
  GetPluginsResult() : CommandResult(CmdName, TargetId) {}

  GetPluginsResult(const std::vector<std::string>& plugins) : CommandResult(CmdName, TargetId) { setPlugins(plugins); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::vector<std::string>>::is_valid(m_values["Plugins"]);
  }
  ...
  std::vector<std::string> plugins() const { return parse_json<std::vector<std::string>>::parse(m_values["Plugins"]); }

  void setPlugins(const std::vector<std::string>& plugins)
  {
    m_values.AddMember("Plugins",
                       parse_json<std::vector<std::string>>::format(plugins, m_values.GetAllocator()),
