...
public:
  ...
  GetPowerOffset() : CommandBase(CmdName, TargetId) {}

  GetPowerOffset(const std::string& signal) : CommandBase(CmdName, TargetId) { setSignal(signal); }

  static GetPowerOffsetPtr create(const std::string& signal) { return std::make_shared<GetPowerOffset>(signal); }

  static GetPowerOffsetPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetPowerOffset>(ptr); }
