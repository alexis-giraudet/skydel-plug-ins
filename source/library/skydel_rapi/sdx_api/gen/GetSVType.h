...
public:
  ...
  GetSVType() : CommandBase(CmdName, TargetId) {}

  GetSVType(const std::string& system, int svId) : CommandBase(CmdName, TargetId)
  {
  ...
  static GetSVTypePtr create(const std::string& system, int svId) { return std::make_shared<GetSVType>(system, svId); }

  static GetSVTypePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetSVType>(ptr); }
