...
public:
  ...
  GetSVTypeResult() : CommandResult(CmdName, TargetId) {}

  GetSVTypeResult(const std::string& system, int svId, const std::string& svType) : CommandResult(CmdName, TargetId)
  {
  ...
  static GetSVTypeResultPtr create(CommandBasePtr relatedCommand,
  ...
  static GetSVTypeResultPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetSVTypeResult>(ptr); }

  virtual bool isValid() const override
  {

    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["System"]) &&
           parse_json<int>::is_valid(m_values["SvId"]) && parse_json<std::string>::is_valid(m_values["SvType"]);
  }
  ...
  std::string svType() const { return parse_json<std::string>::parse(m_values["SvType"]); }

  void setSvType(const std::string& svType)
  {
    m_values.AddMember("SvType",
                       parse_json<std::string>::format(svType, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
