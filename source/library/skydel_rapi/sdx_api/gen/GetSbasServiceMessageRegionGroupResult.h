...
/// --------------- ------------------------------
/// --------------------------------------------------------------------------------- ServiceProvider string Service
/// providers, accepts "WAAS", "EGNOS", "MSAS", "GAGAN" and "SDCM". DeltaUdrei      int                            Delta
/// UDREI applicable inside the defined regions. Accepted range is [0..15]. PriorityCode    int Priority code of the
/// SBAS service message region group. Accepted range is [0..3]. Regions         array SbasServiceMessageRegion Array of
/// SBAS service message regions. Accepted size range is [1..5]. Id              string                         Unique
/// identifier of the SBAS service message region group.
///
...
public:
  ...
  GetSbasServiceMessageRegionGroupResult() : CommandResult(CmdName, TargetId) {}

  GetSbasServiceMessageRegionGroupResult(const std::string& serviceProvider,
                                         int deltaUdrei,
                                         int priorityCode,
                                         const std::vector<Sdx::SbasServiceMessageRegion>& regions,
                                         const std::string& id) :
  ...
  GetSbasServiceMessageRegionGroupResult(CommandBasePtr relatedCommand,
  ...
  static GetSbasServiceMessageRegionGroupResultPtr create(const std::string& serviceProvider,
                                                          int deltaUdrei,
                                                          int priorityCode,
                                                          const std::vector<Sdx::SbasServiceMessageRegion>& regions,
                                                          const std::string& id)
  {
    return std::make_shared<GetSbasServiceMessageRegionGroupResult>(serviceProvider,
                                                                    deltaUdrei,
                                                                    priorityCode,
                                                                    regions,
                                                                    id);
  ...
  static GetSbasServiceMessageRegionGroupResultPtr create(CommandBasePtr relatedCommand,
                                                          const std::string& serviceProvider,
                                                          int deltaUdrei,
                                                          int priorityCode,
                                                          const std::vector<Sdx::SbasServiceMessageRegion>& regions,
                                                          const std::string& id)
  {
    return std::make_shared<GetSbasServiceMessageRegionGroupResult>(relatedCommand,
                                                                    serviceProvider,
                                                                    deltaUdrei,
                                                                    priorityCode,
                                                                    regions,
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["ServiceProvider"]) &&
           parse_json<int>::is_valid(m_values["DeltaUdrei"]) && parse_json<int>::is_valid(m_values["PriorityCode"]) &&
  ...
  int deltaUdrei() const { return parse_json<int>::parse(m_values["DeltaUdrei"]); }

  void setDeltaUdrei(int deltaUdrei)
  {
    m_values.AddMember("DeltaUdrei",
                       parse_json<int>::format(deltaUdrei, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }

  int priorityCode() const { return parse_json<int>::parse(m_values["PriorityCode"]); }

  void setPriorityCode(int priorityCode)
  {
    m_values.AddMember("PriorityCode",
                       parse_json<int>::format(priorityCode, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
  ...
  {
    m_values.AddMember("Regions",
                       parse_json<std::vector<Sdx::SbasServiceMessageRegion>>::format(regions, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
