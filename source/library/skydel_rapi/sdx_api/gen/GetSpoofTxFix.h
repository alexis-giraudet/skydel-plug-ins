...
public:
  ...
  GetSpoofTxFix() : CommandBase(CmdName, TargetId) {}

  GetSpoofTxFix(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static GetSpoofTxFixPtr create(const std::string& id) { return std::make_shared<GetSpoofTxFix>(id); }

  static GetSpoofTxFixPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<GetSpoofTxFix>(ptr); }
