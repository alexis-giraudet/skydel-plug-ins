...
public:
  ...
  GetSyncTimeResult() : CommandResult(CmdName, TargetId) {}

  GetSyncTimeResult(double time) : CommandResult(CmdName, TargetId) { setTime(time); }

  GetSyncTimeResult(CommandBasePtr relatedCommand, double time) : CommandResult(CmdName, TargetId, relatedCommand)
  {

    setTime(time);
  }

  static GetSyncTimeResultPtr create(double time) { return std::make_shared<GetSyncTimeResult>(time); }
