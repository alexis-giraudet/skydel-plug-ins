...
public:
  ...
  GetWFAntennaGainResult() : CommandResult(CmdName, TargetId) {}

  GetWFAntennaGainResult(int gain) : CommandResult(CmdName, TargetId) { setGain(gain); }

  GetWFAntennaGainResult(CommandBasePtr relatedCommand, int gain) : CommandResult(CmdName, TargetId, relatedCommand)
  {

    setGain(gain);
  }

  static GetWFAntennaGainResultPtr create(int gain) { return std::make_shared<GetWFAntennaGainResult>(gain); }
