...
/// ------------------------------- ---------------
/// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/// Element                         int             One-based index of the element. Value -1 adds a new element at the
/// end of the list. Enabled                         bool            If True, this antenna element will be simulated.
/// AntennaModelName                string          Antenna Model name for this element. Antenna models can be defined
/// in Vehicle Antenna Model menu. LnaGain                         optional int    LNA gain to add to the element. If
/// unspecified, default value is 0 dB. Value must be a positive integer between 0 dB and +25 dB. IsGaussianNoiseEnabled
/// optional bool   If true, add Gaussian noise to the GNSS outputs of the element to ensure realistic signal to noise
/// ratio. If unspecified, default value is True. The Gaussian Noise seed used is unique for each output of each element
/// of the Wavefront system. GaussianNoisePowerDensityOffset optional double The Noise Power Density Offset (dB/Hz)
/// added to the base Gaussian Noise power density (-174 dB/Hz) on the GNSS outputs of the element. If Gaussian Noise is
/// not enabled on the element, this offset has no effect. If unspecified, default value is 0 dB/Hz. Value must be
/// between -10 and +10 dB/Hz.
///
...
public:
  ...
  GetWFElementResult(int element,
                     bool enabled,
                     const std::string& antennaModelName,
                     const std::optional<int>& lnaGain = {},
                     const std::optional<bool>& isGaussianNoiseEnabled = {},
                     const std::optional<double>& gaussianNoisePowerDensityOffset = {}) :
  ...
  GetWFElementResult(CommandBasePtr relatedCommand,
                     int element,
                     bool enabled,
                     const std::string& antennaModelName,
                     const std::optional<int>& lnaGain = {},
  ...
  static GetWFElementResultPtr create(int element,
                                      bool enabled,
                                      const std::string& antennaModelName,
                                      const std::optional<int>& lnaGain = {},
                                      const std::optional<bool>& isGaussianNoiseEnabled = {},
                                      const std::optional<double>& gaussianNoisePowerDensityOffset = {})
  {
    return std::make_shared<GetWFElementResult>(element,
                                                enabled,
                                                antennaModelName,
                                                lnaGain,
  ...
  static GetWFElementResultPtr create(CommandBasePtr relatedCommand,
                                      ...
                                      const std::optional<double>& gaussianNoisePowerDensityOffset = {})
  {
    return std::make_shared<GetWFElementResult>(relatedCommand,
                                                element,
                                                enabled,
                                                antennaModelName,
                                                lnaGain,
  ...
  std::optional<int> lnaGain() const { return parse_json<std::optional<int>>::parse(m_values["LnaGain"]); }

  void setLnaGain(const std::optional<int>& lnaGain)
  {
    m_values.AddMember("LnaGain",
                       parse_json<std::optional<int>>::format(lnaGain, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
  ...
  {
    m_values.AddMember("IsGaussianNoiseEnabled",
                       parse_json<std::optional<bool>>::format(isGaussianNoiseEnabled, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
  ...
  {
    m_values.AddMember("GaussianNoisePowerDensityOffset",
                       parse_json<std::optional<double>>::format(gaussianNoisePowerDensityOffset,
                                                                 m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
