...
public:
  ...
  HilPortResult() : CommandResult(CmdName, TargetId) {}

  HilPortResult(int port) : CommandResult(CmdName, TargetId) { setPort(port); }

  HilPortResult(CommandBasePtr relatedCommand, int port) : CommandResult(CmdName, TargetId, relatedCommand)
  {

    setPort(port);
  }

  static HilPortResultPtr create(int port) { return std::make_shared<HilPortResult>(port); }
  ...
  static HilPortResultPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<HilPortResult>(ptr); }

  virtual bool isValid() const override { return m_values.IsObject() && parse_json<int>::is_valid(m_values["Port"]); }
