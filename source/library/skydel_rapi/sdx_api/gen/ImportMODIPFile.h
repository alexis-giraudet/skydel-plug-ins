...
public:
  ...
  ImportMODIPFile() : CommandBase(CmdName, TargetId) {}

  ImportMODIPFile(const std::string& path) : CommandBase(CmdName, TargetId) { setPath(path); }

  static ImportMODIPFilePtr create(const std::string& path) { return std::make_shared<ImportMODIPFile>(path); }

  static ImportMODIPFilePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<ImportMODIPFile>(ptr); }
