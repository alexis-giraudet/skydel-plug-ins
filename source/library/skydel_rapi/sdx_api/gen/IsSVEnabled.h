...
/// or "PULSAR" SvId   int    The satellite's SV ID (use 0 for all SVs).
///
...
public:
  ...
  IsSVEnabled() : CommandBase(CmdName, TargetId) {}

  IsSVEnabled(const std::string& system, int svId) : CommandBase(CmdName, TargetId)
  {
  ...
  static IsSVEnabledPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<IsSVEnabled>(ptr); }
