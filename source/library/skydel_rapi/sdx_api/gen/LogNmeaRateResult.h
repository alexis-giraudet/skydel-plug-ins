...
public:
  ...
  LogNmeaRateResult() : CommandResult(CmdName, TargetId) {}

  LogNmeaRateResult(int rate) : CommandResult(CmdName, TargetId) { setRate(rate); }

  LogNmeaRateResult(CommandBasePtr relatedCommand, int rate) : CommandResult(CmdName, TargetId, relatedCommand)
  {

    setRate(rate);
  }

  static LogNmeaRateResultPtr create(int rate) { return std::make_shared<LogNmeaRateResult>(rate); }
