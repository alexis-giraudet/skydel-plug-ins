...
public:
  ...
  MessageSequenceImport() : CommandBase(CmdName, TargetId) {}

  MessageSequenceImport(const std::string& signal, const std::string& filename) : CommandBase(CmdName, TargetId)
  {
  ...
  std::string filename() const { return parse_json<std::string>::parse(m_values["Filename"]); }

  void setFilename(const std::string& filename)
  {
    m_values.AddMember("Filename",
                       parse_json<std::string>::format(filename, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
