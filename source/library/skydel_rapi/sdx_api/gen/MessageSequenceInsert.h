...
/// ------ ------
/// ----------------------------------------------------------------------------------------------------------- Signal
/// string Signal Name ("L2C" for example) Index  int    Message index in sequence where to insert. Set to -1 to append
/// the message type at the end of the sequence. Type   int    Message type to insert
///
...
public:
  ...
  MessageSequenceInsert() : CommandBase(CmdName, TargetId) {}

  MessageSequenceInsert(const std::string& signal, int index, int type) : CommandBase(CmdName, TargetId)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["Signal"]) &&
           parse_json<int>::is_valid(m_values["Index"]) && parse_json<int>::is_valid(m_values["Type"]);
  }
  ...
  int type() const { return parse_json<int>::parse(m_values["Type"]); }
