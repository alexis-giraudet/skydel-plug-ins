...
public:
  ...
  RemoveAntennaChange() : CommandBase(CmdName, TargetId) {}

  RemoveAntennaChange(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static RemoveAntennaChangePtr create(const std::string& id) { return std::make_shared<RemoveAntennaChange>(id); }
