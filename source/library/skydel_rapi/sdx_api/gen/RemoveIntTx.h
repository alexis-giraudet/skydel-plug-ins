...
public:
  ...
  RemoveIntTx() : CommandBase(CmdName, TargetId) {}

  RemoveIntTx(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static RemoveIntTxPtr create(const std::string& id) { return std::make_shared<RemoveIntTx>(id); }

  static RemoveIntTxPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<RemoveIntTx>(ptr); }
