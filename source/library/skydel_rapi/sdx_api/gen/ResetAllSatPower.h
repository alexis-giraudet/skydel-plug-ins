...
public:
  ...
  ResetAllSatPower() : CommandBase(CmdName, TargetId) {}

  ResetAllSatPower(const std::string& system) : CommandBase(CmdName, TargetId) { setSystem(system); }

  static ResetAllSatPowerPtr create(const std::string& system) { return std::make_shared<ResetAllSatPower>(system); }
