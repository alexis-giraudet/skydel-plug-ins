...
/// --------- ------
/// --------------------------------------------------------------------------------------------------------- Path
/// string Configuration path. Automatically add file suffix if missing. If folder not defined, user default folder.
...
public:
  ...
  SaveAs() : CommandBase(CmdName, TargetId) {}

  SaveAs(const std::string& path, bool overwrite) : CommandBase(CmdName, TargetId)
  {
  ...
  static SaveAsPtr create(const std::string& path, bool overwrite) { return std::make_shared<SaveAs>(path, overwrite); }

  static SaveAsPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SaveAs>(ptr); }
  ...
  bool overwrite() const { return parse_json<bool>::parse(m_values["Overwrite"]); }

  void setOverwrite(bool overwrite)
  {
    m_values.AddMember("Overwrite",
                       parse_json<bool>::format(overwrite, m_values.GetAllocator()),
