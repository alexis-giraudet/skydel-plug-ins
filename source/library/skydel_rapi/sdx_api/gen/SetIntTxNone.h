...
public:
  ...
  SetIntTxNone() : CommandBase(CmdName, TargetId) {}

  SetIntTxNone(const std::string& id) : CommandBase(CmdName, TargetId) { setId(id); }

  static SetIntTxNonePtr create(const std::string& id) { return std::make_shared<SetIntTxNone>(id); }

  static SetIntTxNonePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetIntTxNone>(ptr); }
