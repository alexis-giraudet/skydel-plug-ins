...
{
///
/// Set the manual propagation loss for the given interference signal. Value is used only if
/// SetIntTxUseManualPropagationLoss has been set on the corresponding transmitter. This value is not preserved after
/// simulation end.
///
/// Name            Type   Description
/// --------------- ------
/// --------------------------------------------------------------------------------------------------------
/// PropagationLoss double The propagation loss in dB to use until the next call of this command, or until manual mode
/// is disabled. TransmitterId   string Transmitter unique identifier. SignalId        string Signal unique identifier.
///
...
public:
  ...
  SetIntTxSignalManualPropagationLoss() : CommandBase(CmdName, TargetId) {}

  SetIntTxSignalManualPropagationLoss(double propagationLoss,
  ...
  static SetIntTxSignalManualPropagationLossPtr create(double propagationLoss,
                                                       const std::string& transmitterId,
                                                       const std::string& signalId)
  {
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<double>::is_valid(m_values["PropagationLoss"]) &&
  ...
  double propagationLoss() const { return parse_json<double>::parse(m_values["PropagationLoss"]); }

  void setPropagationLoss(double propagationLoss)
  {
    m_values.AddMember("PropagationLoss",
                       parse_json<double>::format(propagationLoss, m_values.GetAllocator()),
