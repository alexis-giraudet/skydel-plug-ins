...
{
///
/// Set whether a dynamic transmitter should only use user-provided propagation loss values. Propagation loss will not
/// update automatically if set. This state is not preserved after simulation end.
///
...
public:
  ...
  SetIntTxUseManualPropagationLoss() : CommandBase(CmdName, TargetId) {}

  SetIntTxUseManualPropagationLoss(bool manual, const std::string& id) : CommandBase(CmdName, TargetId)
  {
