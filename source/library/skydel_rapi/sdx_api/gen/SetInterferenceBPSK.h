...
/// relative to nominal power. Minimum = -40 dB, Maximum = 43 dB. CodeRate     int          Code rate (Chips/s). Must be
/// a multiple of 1 kChips/s. Minimum = 1000 Chips/s (1 kChips/s), Maximum = 60000000 Chips/s (60 MChips/s).
...
/// Prn          optional int PRN code index to use in the BPSK modulation. If zero, a random code will be used. Minimum
/// = 0, Maximum = 32.
///
...
public:
  ...
  SetInterferenceBPSK() : CommandBase(CmdName, TargetId) {}

  SetInterferenceBPSK(int startTime,
                      int stopTime,
                      double centralFreq,
                      double power,
                      int codeRate,
  ...
  static SetInterferenceBPSKPtr create(int startTime,
                                       int stopTime,
                                       double centralFreq,
                                       double power,
                                       int codeRate,
                                       ...
                                       const std::optional<int>& prn = {})
  {
    return std::make_shared<SetInterferenceBPSK>(startTime,
                                                 stopTime,
                                                 centralFreq,
                                                 power,
                                                 codeRate,
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<int>::is_valid(m_values["StartTime"]) &&
           ...
           parse_json<int>::is_valid(m_values["CodeLengthMs"]) && parse_json<bool>::is_valid(m_values["Enabled"]) &&
  ...
  virtual const std::vector<std::string>& fieldNames() const override
  {
    static const std::vector<std::string>
      names {"StartTime", "StopTime", "CentralFreq", "Power", "CodeRate", "CodeLengthMs", "Enabled", "Id", "Prn"};
    return names;
  }
