...
{
///
/// Set Error offsets in the ionospheric grid.  The array is zero based, the index 0 in a band array is for the IGP with
/// an index 1, etc.
///
...
public:
  ...
  SetIonoGridErrorAll() : CommandBase(CmdName, TargetId) {}

  SetIonoGridErrorAll(const std::vector<std::vector<double>>& grid) : CommandBase(CmdName, TargetId) { setGrid(grid); }
