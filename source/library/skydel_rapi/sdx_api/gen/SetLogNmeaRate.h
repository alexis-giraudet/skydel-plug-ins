...
public:
  ...
  SetLogNmeaRate() : CommandBase(CmdName, TargetId) {}

  SetLogNmeaRate(int rate) : CommandBase(CmdName, TargetId) { setRate(rate); }

  static SetLogNmeaRatePtr create(int rate) { return std::make_shared<SetLogNmeaRate>(rate); }

  static SetLogNmeaRatePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetLogNmeaRate>(ptr); }
