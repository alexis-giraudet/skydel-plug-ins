...
public:
  ...
  SetLogRawRate() : CommandBase(CmdName, TargetId) {}

  SetLogRawRate(int rate) : CommandBase(CmdName, TargetId) { setRate(rate); }

  static SetLogRawRatePtr create(int rate) { return std::make_shared<SetLogRawRate>(rate); }

  static SetLogRawRatePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetLogRawRate>(ptr); }
