...
{
///
/// Add or update signal echo (multipath). Offsets (power loss, pseudorange, Doppler and carrier phase) are all relative
/// to line-of-sight signal. If Id is not set, or unknown to Skydel, a new echo will be added. Otherwise, existing echo
/// will be updated.
///
...
///                                           "QZSSL5S", "QZSSL6", "NAVICL1", "NAVICL5", "NAVICS", "PULSARXL" and
///                                           "PULSARX1"
...
public:
  ...
  SetMultipathForSV() : CommandBase(CmdName, TargetId) {}

  SetMultipathForSV(const std::string& signal,
                    int svId,
                    double powerLoss,
                    double pseudorange,
                    double doppler,
  ...
  static SetMultipathForSVPtr create(const std::string& signal,
                                     int svId,
                                     double powerLoss,
                                     double pseudorange,
                                     double doppler,
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["Signal"]) &&
           parse_json<int>::is_valid(m_values["SvId"]) && parse_json<double>::is_valid(m_values["PowerLoss"]) &&
  ...
  virtual const std::vector<std::string>& fieldNames() const override
  {
    static const std::vector<std::string>
      names {"Signal", "SvId", "PowerLoss", "Pseudorange", "Doppler", "CarrierPhase", "Echo", "Id"};
    return names;
  }
