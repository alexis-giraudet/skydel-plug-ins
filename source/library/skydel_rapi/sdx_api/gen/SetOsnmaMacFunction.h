...
/// ----------- ------
/// -------------------------------------------------------------------------------------------------------- MacFunction
/// string MAC function used to authenticate the navigation data. MAC functions are: "HMAC-SHA-256" and "CMAC-AES".
...
public:
  ...
  SetOsnmaMacFunction() : CommandBase(CmdName, TargetId) {}

  SetOsnmaMacFunction(const std::string& macFunction) : CommandBase(CmdName, TargetId) { setMacFunction(macFunction); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["MacFunction"]);
  }
  ...
  std::string macFunction() const { return parse_json<std::string>::parse(m_values["MacFunction"]); }

  void setMacFunction(const std::string& macFunction)
  {
    m_values.AddMember("MacFunction",
                       parse_json<std::string>::format(macFunction, m_values.GetAllocator()),
