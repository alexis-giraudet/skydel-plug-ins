...
public:
  ...
  SetOsnmaTeslaKeySize() : CommandBase(CmdName, TargetId) {}

  SetOsnmaTeslaKeySize(int keySize) : CommandBase(CmdName, TargetId) { setKeySize(keySize); }

  static SetOsnmaTeslaKeySizePtr create(int keySize) { return std::make_shared<SetOsnmaTeslaKeySize>(keySize); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<int>::is_valid(m_values["KeySize"]);
  }
  ...
  int keySize() const { return parse_json<int>::parse(m_values["KeySize"]); }
