...
public:
  ...
  SetPilotPRN() : CommandBase(CmdName, TargetId) {}

  SetPilotPRN(bool enabled,
              int outputIdx,
              double centralFreqOffset,
              double power,
              int prn,
  ...
  static SetPilotPRNPtr create(bool enabled,
                               int outputIdx,
                               double centralFreqOffset,
                               double power,
                               int prn,
  ...
  static SetPilotPRNPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetPilotPRN>(ptr); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<bool>::is_valid(m_values["Enabled"]) &&
           ...
           parse_json<double>::is_valid(m_values["Power"]) && parse_json<int>::is_valid(m_values["Prn"]) &&
  ...
  virtual const std::vector<std::string>& fieldNames() const override
  {
    static const std::vector<std::string>
      names {"Enabled", "OutputIdx", "CentralFreqOffset", "Power", "Prn", "Type", "PilotId"};
    return names;
  }
  ...
  double centralFreqOffset() const { return parse_json<double>::parse(m_values["CentralFreqOffset"]); }

  void setCentralFreqOffset(double centralFreqOffset)
  {
    m_values.AddMember("CentralFreqOffset",
                       parse_json<double>::format(centralFreqOffset, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
  ...
  std::string pilotId() const { return parse_json<std::string>::parse(m_values["PilotId"]); }

  void setPilotId(const std::string& pilotId)
  {
    m_values.AddMember("PilotId",
                       parse_json<std::string>::format(pilotId, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
