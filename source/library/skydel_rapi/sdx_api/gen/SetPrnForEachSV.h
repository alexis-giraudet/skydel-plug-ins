...
/// "NAVICL1", "NAVICL5", "NAVICS", "PULSARXL", "PULSARX1" Prn    array int PRN value to set for each satellite. Zero
/// based index (index 0 => PRN for SV ID 1, index 1 => PRN for SV ID 2, etc)
///
...
public:
  ...
  SetPrnForEachSV() : CommandBase(CmdName, TargetId) {}

  SetPrnForEachSV(const std::string& signal, const std::vector<int>& prn) : CommandBase(CmdName, TargetId)
  {
  ...
  static SetPrnForEachSVPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetPrnForEachSV>(ptr); }
