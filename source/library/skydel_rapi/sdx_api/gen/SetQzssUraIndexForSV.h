...
namespace Sdx
{
  namespace Cmd
  {
    ...
    {
    public:
      inline static const char* const CmdName = "SetQzssUraIndexForSV";
      inline static const char* const Documentation = "Set the URA index of a QZSS satellite\n"      "\n"      "Name        Type            Description\n"      "----------- --------------- -------------------------------------------------------------------------------------------\n"      "SvId        int             The satellite SV ID, or use 0 to apply new value to all satellites.\n"      "Urai        int             URA index.\n"      "DataSetName optional string Optional name of the data set to use. If no value is provided, the active data set is used.";
    ...
    REGISTER_COMMAND_TO_FACTORY(SetQzssUraIndexForSV);
  }
}
