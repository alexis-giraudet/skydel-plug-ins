...
public:
  ...
  SetRFOutputOffsets() : CommandBase(CmdName, TargetId) {}

  SetRFOutputOffsets(int outputIdx,
                     double powerOffset,
                     double timeOffset,
                     double phaseOffset,
                     const std::optional<double>& phaseStep = {}) :
  ...
  static SetRFOutputOffsetsPtr create(int outputIdx,
                                      double powerOffset,
                                      double timeOffset,
                                      double phaseOffset,
                                      const std::optional<double>& phaseStep = {})
  ...
  double powerOffset() const { return parse_json<double>::parse(m_values["PowerOffset"]); }

  void setPowerOffset(double powerOffset)
  {
    m_values.AddMember("PowerOffset",
                       parse_json<double>::format(powerOffset, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }

  double timeOffset() const { return parse_json<double>::parse(m_values["TimeOffset"]); }

  void setTimeOffset(double timeOffset)
  {
    m_values.AddMember("TimeOffset",
                       parse_json<double>::format(timeOffset, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
  ...
  std::optional<double> phaseStep() const { return parse_json<std::optional<double>>::parse(m_values["PhaseStep"]); }

  void setPhaseStep(const std::optional<double>& phaseStep)
  {
    m_values.AddMember("PhaseStep",
                       parse_json<std::optional<double>>::format(phaseStep, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
