...
public:
  ...
  SetRfGain() : CommandBase(CmdName, TargetId) {}

  SetRfGain(int output, double gain) : CommandBase(CmdName, TargetId)
  {
  ...
  static SetRfGainPtr create(int output, double gain) { return std::make_shared<SetRfGain>(output, gain); }

  static SetRfGainPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetRfGain>(ptr); }
  ...
  double gain() const { return parse_json<double>::parse(m_values["Gain"]); }
