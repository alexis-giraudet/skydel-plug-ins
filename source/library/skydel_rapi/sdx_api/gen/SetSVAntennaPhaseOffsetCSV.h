...
{
///
/// Set space vehicle phase offset antenna pattern from a CSV file. If no name is specified, the command is aplied to
/// Basic SV Antenna.
///
/// Name       Type                   Description
/// ---------- ----------------------
/// ------------------------------------------------------------------------------------------------------------------
/// FilePath   string                 File path of the CSV. For Default and None type, lets it empty. See formats in
/// user manual part 8.7.9.2.1. Models. FileFormat AntennaPatternFileType Values' unit of the CSV File: "Degrees",
/// "Radians" or "NoneFile" in case of None-pattern type. Type       AntennaPatternType     Pattern type Band GNSSBand
/// Frequency band System     string                 "GPS", "GLONASS", "Galileo", "BeiDou", "SBAS", "QZSS", "NavIC" or
/// "PULSAR" Name       optional string        SV antenna name
///
...
public:
  ...
  SetSVAntennaPhaseOffsetCSV() : CommandBase(CmdName, TargetId) {}

  SetSVAntennaPhaseOffsetCSV(const std::string& filePath,
                             const Sdx::AntennaPatternFileType& fileFormat,
  ...
  static SetSVAntennaPhaseOffsetCSVPtr create(const std::string& filePath,
                                              const Sdx::AntennaPatternFileType& fileFormat,
                                              const Sdx::AntennaPatternType& type,
                                              const Sdx::GNSSBand& band,
                                              const std::string& system,
  ...
  {
    m_values.AddMember("FileFormat",
                       parse_json<Sdx::AntennaPatternFileType>::format(fileFormat, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
