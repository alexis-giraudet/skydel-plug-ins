...
{
///
/// When SetStatusLogMaxEntriesCountMode is set to "Custom", this commands sets the maximum number of entries in the
/// Status Log. Default value is 1000 entries.
///
...
public:
  ...
  SetStatusLogMaxEntriesCount() : CommandBase(CmdName, TargetId) {}

  SetStatusLogMaxEntriesCount(int maxCount) : CommandBase(CmdName, TargetId) { setMaxCount(maxCount); }
  ...
  virtual bool isValid() const override
  {
    ...
    return m_values.IsObject() && parse_json<int>::is_valid(m_values["MaxCount"]);
  }
  ...
  int maxCount() const { return parse_json<int>::parse(m_values["MaxCount"]); }
