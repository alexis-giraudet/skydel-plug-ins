...
public:
  ...
  SetStreamingBuffer() : CommandBase(CmdName, TargetId) {}

  SetStreamingBuffer(int size) : CommandBase(CmdName, TargetId) { setSize(size); }

  static SetStreamingBufferPtr create(int size) { return std::make_shared<SetStreamingBuffer>(size); }
