...
public:
  ...
  SetSyncServer() : CommandBase(CmdName, TargetId) {}

  SetSyncServer(int port) : CommandBase(CmdName, TargetId) { setPort(port); }

  static SetSyncServerPtr create(int port) { return std::make_shared<SetSyncServer>(port); }

  static SetSyncServerPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetSyncServer>(ptr); }
