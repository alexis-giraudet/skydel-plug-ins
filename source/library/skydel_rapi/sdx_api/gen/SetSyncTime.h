...
public:
  ...
  SetSyncTime() : CommandBase(CmdName, TargetId) {}

  SetSyncTime(double time) : CommandBase(CmdName, TargetId) { setTime(time); }

  static SetSyncTimePtr create(double time) { return std::make_shared<SetSyncTime>(time); }

  static SetSyncTimePtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetSyncTime>(ptr); }
