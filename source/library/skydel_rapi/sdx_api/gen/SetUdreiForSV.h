...
public:
  ...
  SetUdreiForSV() : CommandBase(CmdName, TargetId) {}

  SetUdreiForSV(const std::string& system, int svId, int udrei) : CommandBase(CmdName, TargetId)
  {
  ...
  static SetUdreiForSVPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<SetUdreiForSV>(ptr); }

  virtual bool isValid() const override
  {

    return m_values.IsObject() && parse_json<std::string>::is_valid(m_values["System"]) &&
           parse_json<int>::is_valid(m_values["SvId"]) && parse_json<int>::is_valid(m_values["Udrei"]);
  }
  ...
  int udrei() const { return parse_json<int>::parse(m_values["Udrei"]); }
