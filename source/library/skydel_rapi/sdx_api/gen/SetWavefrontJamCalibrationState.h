...
public:
  ...
  SetWavefrontJamCalibrationState() : CommandBase(CmdName, TargetId) {}

  SetWavefrontJamCalibrationState(bool state) : CommandBase(CmdName, TargetId) { setState(state); }
  ...
  virtual bool isValid() const override { return m_values.IsObject() && parse_json<bool>::is_valid(m_values["State"]); }
  ...
  bool state() const { return parse_json<bool>::parse(m_values["State"]); }
