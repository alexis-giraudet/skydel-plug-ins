...
/// ------------------- ------
/// -----------------------------------------------------------------------------------------------------------
/// AntennaOffset       double The receiver antenna power offset (dB). It depends on antenna pattern and relative
/// orientation with signal. StrengthModelOffset double The power offset coming from the Signal Strength Model (dB).
...
/// Total               double The sum of all the other fields, including nominal power and the global power offset
/// (dBm).
///
...
struct parse_json<Sdx::SignalPower>
{
  static bool is_valid(const rapidjson::Value& value)
  {
    ...
    return value.IsObject() && parse_json<double>::is_valid(value["AntennaOffset"]) &&
           parse_json<double>::is_valid(value["StrengthModelOffset"]) &&
           parse_json<double>::is_valid(value["SignalOffset"]) && parse_json<double>::is_valid(value["ManualOffset"]) &&
