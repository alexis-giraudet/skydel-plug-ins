...
public:
  ...
  StartPPS() : CommandBase(CmdName, TargetId) {}

  StartPPS(int msec) : CommandBase(CmdName, TargetId) { setMsec(msec); }

  static StartPPSPtr create(int msec) { return std::make_shared<StartPPS>(msec); }

  static StartPPSPtr dynamicCast(CommandBasePtr ptr) { return std::dynamic_pointer_cast<StartPPS>(ptr); }

  virtual bool isValid() const override { return m_values.IsObject() && parse_json<int>::is_valid(m_values["Msec"]); }
  ...
  int msec() const { return parse_json<int>::parse(m_values["Msec"]); }
