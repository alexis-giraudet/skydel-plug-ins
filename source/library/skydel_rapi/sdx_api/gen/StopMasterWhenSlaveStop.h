...
{
///
/// Please note the command StopMasterWhenSlaveStop is deprecated since 23.11. You may use
...
public:
  ...
  StopMasterWhenSlaveStop() : CommandBase(CmdName, TargetId) {}

  StopMasterWhenSlaveStop(bool enabled) : CommandBase(CmdName, TargetId) { setEnabled(enabled); }

  static StopMasterWhenSlaveStopPtr create(bool enabled) { return std::make_shared<StopMasterWhenSlaveStop>(enabled); }
