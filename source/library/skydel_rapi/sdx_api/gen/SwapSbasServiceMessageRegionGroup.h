...
public:
  ...
  SwapSbasServiceMessageRegionGroup() : CommandBase(CmdName, TargetId) {}

  SwapSbasServiceMessageRegionGroup(const std::string& serviceProvider,
  ...
  static SwapSbasServiceMessageRegionGroupPtr create(const std::string& serviceProvider,
                                                     const std::string& firstId,
                                                     const std::string& secondId)
  {
  ...
  std::string firstId() const { return parse_json<std::string>::parse(m_values["FirstId"]); }

  void setFirstId(const std::string& firstId)
  {
    m_values.AddMember("FirstId",
                       parse_json<std::string>::format(firstId, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }

  std::string secondId() const { return parse_json<std::string>::parse(m_values["SecondId"]); }

  void setSecondId(const std::string& secondId)
  {
    m_values.AddMember("SecondId",
                       parse_json<std::string>::format(secondId, m_values.GetAllocator()),
                       m_values.GetAllocator());
  }
};
