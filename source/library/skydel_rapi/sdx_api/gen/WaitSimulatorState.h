...
namespace Sdx
{
  namespace Cmd
  {
    ...
    public:
      ...
      inline static const char* const Documentation = "Wait until simulator has reached the specified substate unless simulator goes to error state or specified failure substate. Will return a SimulatorStateResult.\n"      "Possible substates are :\n"      "-None\n"      "-Incomplete\n"      "-Ready\n"      "-Initializing\n"      "-Armed\n"      "-Streaming RF\n"      "-Sync Worker Instance\n"      "-WF Init (Worker)\n"      "-WF Init (Main)\n"      "-HIL Sync\n"      "-Sync Init\n"      "-Sync PPS Reset\n"      "-Sync Start Time\n"      "-Sync Start\n"      "-Error\n"      "\n"      "Name         Type   Description\n"      "------------ ------ ----------------------------------------------------------\n"      "State        string Simulator sub state string.\n"      "FailureState string Abort waiting if simulator goes to this sub state instead.";
